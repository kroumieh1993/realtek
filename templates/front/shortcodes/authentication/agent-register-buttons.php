<?php /** @var $args array */ ?>
<div class="es-auth__item es-auth__agent-register-buttons <?php echo $args['auth_item'] != 'agent-register-buttons' ? 'es-auth__item--hidden' : ''; ?>">
	...
	foreach ( es_get_auth_networks_list() as $network ) : ?>
		<?php if ( ! empty( $args[ 'enable_' . $network ] ) ) :
			$auth = es_get_auth_instance( $network, array(
				'context' => 'agent-register-buttons'
			) );
	...
	if ( ! empty( $args['enable_login_form'] ) ) : ?>
		<a href="#" data-auth-item="agent-register-form" class="js-es-auth-item__switcher es-btn es-btn--default es-btn--auth "><?php _e( 'Sign up with email', 'es' ); ?></a>
	<?php endif; ?>
