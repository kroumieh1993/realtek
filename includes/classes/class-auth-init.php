...
class Es_Auth {
	...
	public static function init() {
		...
		add_action( 'social_register_new_user', array( 'Es_Auth', 'create_agent_post' ), 10, 2 );
		...
		add_action( 'es_register_new_agent', array( 'Es_Auth', 'register_agent' ) );
		add_filter( 'authenticate', array( 'Es_Auth', 'validate_login' ), 20, 2 );
		add_filter( 'init', array( 'Es_Auth', 'approve_user' ) );
	}

	/**
	 * Approve user using confirmation link after registration action.
	 *
	 * @return void
	 */
	public static function approve_user() {
		$action = es_get( 'auth-action' );
		$email = es_get( 'user_email' );
		$key = es_get( 'k' );

		if ( 'approve-user' == $action && ! empty( $email ) && ! empty( $key ) ) {
			$user = get_user_by_email( $email );
			$activated = 0;

			if ( $user instanceof WP_User && ( $user_instance = es_get_user_entity( $user->ID ) ) ) {
				if ( $user_instance->is_active() ) {
					es_set_flash( 'authenticate', __( 'User already activated.', 'es' ) );
					$activated = 1;
				} else if ( $user_instance->approval_key == $key ) {
					$user_instance->activate();
					$user_instance->delete_field_value( 'approval_key' );
					es_set_flash( 'authenticate', __( 'User successfully approved.', 'es' ) );
					$activated = 1;
				}
			}

			if ( ! $activated ) {
				es_set_flash( 'authenticate', __( 'Invalid user or secure token.', 'es' ), 'error' );
			}

			wp_safe_redirect( es_get_page_url( 'login' ) );
			die;
		}
	}
	 ...
	 */
	public static function validate_login( $user, $username ) {
		if ( $user instanceof WP_User && $user->has_cap( 'agent' ) ) {
			$user_instance = es_get_user_entity( $user->ID );

			if ( ! $user_instance->is_active() ) {
				if ( $user_instance->is_unapproved() ) {
					$user = new WP_Error( 'not_activated_user', __( 'The user is not approved. Please confirm your registration via email.', 'es' ) );
				}

				if ( $user_instance->is_disabled() ) {
					$user = new WP_Error( 'disabled_user', __( 'The user is deactivated. Please, contact the support.' ) );
				}
			}
		}
	 ...
	 *
	 * @param $user_id int
	 * @param $auth_instance Es_Authentication
	 */
	public static function create_agent_post( $user_id, $auth_instance ) {
		$auth_network = es_clean( filter_input( INPUT_GET, 'auth_network' ) );

		if ( stristr( $auth_network, 'agent' ) ) {
			...
			$agent_post_id = es_save_agent( array(
				'system' => array(
					'post_title' => $agent_user->get_full_name(),
					'post_status' => 'publish'
				),
			) );

			if ( ! is_wp_error( $agent_post_id ) && $agent_post_id ) {
				$agent_post = es_get_agent( $agent_post_id );
				$agent_user->activate();

				$agent_post->save_field_value( 'has_user', 1 );
				$agent_post->save_field_value( 'user_id', $user_id );
			}
		}
	}
	 ...
	 * @param $user_data
	 */
	public static function register_agent( $user_data ) {
		$user_data['has_user'] = true;
		$user_data['post_title'] = $user_data['user_email'];
		$user_data['post_status'] = 'draft';
		$user_data['status'] = Es_Agent_User::STATUS_UNAPPROVED;

		if ( ! empty( $_POST['es_phone'] ) ) {
			if ( is_string( $_POST['es_phone'] ) ) {
				$tel = es_prepare_tel( sanitize_text_field( $_POST['es_phone'] ) );
			} else if ( ! empty( $_POST['es_phone']['tel'] ) ) {
				$tel = $_POST['es_phone'];
			}

			if ( ! empty( $tel ) ) {
				$user_data['contacts'][] = array(
					'phone' =>  es_clean( $_POST['es_phone'] ),
		...
		$user_data = apply_filters( 'es_before_register_agent_data', $user_data );

		$agent_post_id = es_save_agent( $user_data );

		if ( $agent_post_id && ! is_wp_error( $agent_post_id ) ) {
			$entity = es_get_agent( $agent_post_id );
			$user = $entity->get_user_entity();

			if ( $user instanceof Es_User && $user->get_id() ) {
				static::$new_user_id = $user->get_id();
			} else {
				wp_delete_post( $agent_post_id, true );
				static::$new_user_id = new WP_Error( 'Agent is not created. Please contact the support.' );
			}
		} else {
			static::$new_user_id = $agent_post_id;
		}
	}
	...
	public static function register() {
		...
		if ( wp_verify_nonce( es_get_nonce( 'es_register_nonce_' . $uniqid ), 'es_register' ) ) {
			$type = filter_input( INPUT_POST, 'es_type' );
	...
	public static function send_new_user_notifications( $user_id, $notify = 'both' ) {
		...
		if ( $user->has_cap( 'agent' ) ) {
			es_send_email( 'new_agent_confirm', $user->user_email, array(
				'user_login' => $user->user_login,
				'user_email' => $user->user_email,
			) );
		} else {
