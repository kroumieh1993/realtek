...
class Es_Migrations {
	...
	public static function execute() {
		...
		if ( ! static::is_executed( 'pro_roles' ) ) {
			$capabilities = array(
				'read_agent',
				'edit_agent',
				'create_agents',
				...
				'create_es_requests',
				'edit_es_requests',
				'edit_published_es_requests',
				'delete_published_es_requests',
				'delete_es_request',
				'publish_es_requests',
				'edit_others_es_requests',
				'read_private_es_requests',
				'delete_es_requests',
				'delete_private_es_requests',
				'delete_others_es_requests',
				'edit_private_es_requests',

				'read_agency',
				'edit_agency',
				'create_agencies',
				'edit_agencies',
				'edit_published_agencies',
				'delete_published_agencies',
				'delete_agency',
				'publish_agencies',
				'edit_others_agencies',
				'read_private_agencies',
				'delete_agencies',
				'delete_private_agencies',
				'delete_others_agencies',
				'edit_private_agencies',
				'copy_properties',

				'view_property_management',
			);

			$admin_role = get_role( 'administrator' );

			foreach ( $capabilities as $cap ) {
				if ( $admin_role->has_cap( $cap ) ) continue;
			...
			foreach ( $capabilities as $cap ) {
				if ( $agent_role->has_cap( $cap ) ) continue;

				$agent_role->add_cap( $cap );
			}

			if ( $admin_role->has_cap( 'edit_private_es_properties' ) && $agent_role->has_cap( 'edit_es_property' ) ) {
				static::set_executed( 'pro_roles' );
			}
		}
