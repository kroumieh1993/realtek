...
//			),
	 ...
	 * @param $user_id
	 */
	public static function migrate_agent( $user_id ) {
		$user = get_user_by( 'id', $user_id );

		if ( $user instanceof WP_User ) {
			$user->add_role( 'agent' );
			$status = get_user_meta( $user_id, 'es_agent_status', true );

			if ( $status ) {
				update_user_meta( $user_id, 'es_agent_user_status', $status );
			}

			$profile_attachment_id = get_user_meta( $user_id, 'es_agent_profile_attachment_id', true );

			if ( $profile_attachment_id ) {
				update_user_meta( $user_id, 'es_agent_user_avatar_id', $profile_attachment_id );
			}

			$name = get_user_meta( $user_id, 'es_agent_name', true );

			$agent_data = array(
				'system' => array(
					'post_title' => $name ? $name : $user->user_login,
					'ID' => get_user_meta( $user_id, 'es_agent_user_post_id', true ),
					'post_content' => get_user_meta( $user_id, 'es_agent_about', true ),
				),
				'has_user' => false,
				'user_id' => $user_id,
			);
			...
			if ( $user->first_name && $user->last_name ) {
				$agent_data['first_name'] = $user->first_name;
				$agent_data['last_name'] = $user->last_name;
				$agent_data['system']['post_title'] = $user->first_name . ' ' . $user->last_name;
			} else {
				$name_exploded = $name ? explode( ' ', trim( $name ) ) : false;
				if ( $name_exploded && count( $name_exploded ) == 2 ) {
					$agent_data['first_name'] = $name_exploded[0];
					$agent_data['last_name'] = $name_exploded[1];
				}
			}

			$post_id = es_save_agent( $agent_data );

			if ( ! is_wp_error( $post_id ) ) {
				$agent = es_get_agent();
				$agent->save_field_value( 'facebook', get_user_meta( $user_id, 'es_agent_facebook_url', true ) );
				$agent->save_field_value( 'twitter', get_user_meta( $user_id, 'es_agent_twitter_url', true ) );
				$agent->save_field_value( 'pinterest', get_user_meta( $user_id, 'es_agent_pinterest_url', true ) );
				$agent->save_field_value( 'youtube', get_user_meta( $user_id, 'es_agent_youtube_url', true ) );

				if ( $profile_attachment_id ) {
					set_post_thumbnail( $post_id, $profile_attachment_id );
				}

				if ( $tel = get_user_meta( $user_id, 'es_agent_tel', true ) ) {
					$agent->save_field_value( 'contacts', array(
						array(
							'phone' => array(
								'code' => '',
								'tel' => $tel,
