...
class Es_Fields_Builder extends Es_Fields_Builder_Item {
	...
	public static function get_types_list() {
		return apply_filters( 'es_fields_builder_get_types_list', array(
			...
			'editor' => _x( 'HTML Editor', 'field type', 'es' ),
	 ...
	 * @return stdClass[]
	 */
	public static function get_frontend_tab_fields( $section_machine_name, $entity = 'property' ) {
		$fields = static::get_items( $entity );

		foreach ( $fields as $key => $field ) {
			if ( ! empty( $field['disable_on_frontend'] ) ) {
				unset( $fields[ $key ] );
				continue;
			}

			if ( ! empty( $field['frontend_tab_machine_name'] ) ) {
				unset( $fields[ $key ]['tab_machine_name'] );
			}
		}

		if ( ! empty( $fields ) ) {
			$fields = wp_filter_object_list( $fields, array(
				'frontend_tab_machine_name' => $section_machine_name,
				'tab_machine_name'          => $section_machine_name,
			), 'or' );
		}

		return apply_filters( 'es_fb_get_frontend_tab_fields', $fields, $section_machine_name, $entity );
	...
	public static function prepare_item_data( $section_data ) {
		$valid_cols = array( 'id', 'label', 'frontend_form_name', 'frontend_machine_name', 'type', 'values', 'options',
			'machine_name', 'section_machine_name', 'tab_machine_name', 'entity_name', 'mandatory', 'search_support',
			'compare_support', 'mls_import_support', 'is_visible', 'is_pdf_visible', 'is_visible_for', 'order',
			'address_component', 'is_full_width' );
