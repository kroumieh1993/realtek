 ...
 */
class Es_My_Agencies_Shortcode extends Es_My_Entities_Shortcode {
	...
	public function get_content() {
		...
		if ( $attr['type'] == 'agent' ) {
			$instance = es_get_shortcode_instance( 'es_my_agents', $attr );
	...
	public function get_wrapper_class() {
		$classes = array( 'es-agencies', 'js-es-agencies-wrap', 'js-es-entities-wrap' );

		return apply_filters( 'es_my_agencies_wrapper_classes', implode( ' ', $classes ), $classes, $this );
	}
	...
	public function get_query_args() {
		...
		if ( ! empty( $this->_attributes['agencies_id'] ) ) {
			$args['query']['post__in'] = array_map( 'trim', explode( ',', $this->_attributes['agencies_id'] ) );
		}

		$query = apply_filters( 'es_agencies_shortcode_query_args', es_get_agencies_query_args( $args ), $this->_attributes, $this );
	...
	public function get_default_attributes() {
		...
		return apply_filters( sprintf( '%s_default_attributes', $shortcode_name[0] ), array(
			'layout' => ests( 'agencies_layout' ),
			'posts_per_page' => ests( 'agencies_per_page' ),
			'disable_navbar' => false,
			'show_sort' => ests( 'is_agencies_sorting_enabled' ),
			'show_total' => true,
			'show_page_title' => true,
			'show_layouts' => ests( 'is_agencies_layout_switcher_enabled' ),
			'sort' => ests( 'agencies_default_sorting_option' ),
			'limit' => 0,
			'page_num' => null,
			'agencies_id' => '',
			'type' => 'agency',
	...
	public static function get_shortcode_name() {
		return array( 'es_my_agencies', 'es_agencies', 'agencies' );
	}
}
