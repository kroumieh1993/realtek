        ...
        if ( ! empty( $section['machine_name'] ) ) {
            ...
            switch ( $section['machine_name'] ) {
                ...
                case 'request_form':
                    ...
                    if ( ! empty( $section['options']['recipient_type'] ) ) {
	                    $config['recipient_type'] = $section['options']['recipient_type'];
                    }
	                if ( ! empty( $section['options']['custom_email'] ) ) {
		                $config['custom_email'] = $section['options']['custom_email'];
	                }
...
if ( ! function_exists( 'es_the_entity_section' ) ) {
    ...
    function es_the_entity_section( $section, $post_id = 0 ) {
	    ...
	    $section['entity_name'] = $entity_name;
        if ( es_is_visible( $section, $entity_name, 'section' ) ) {
            $section[ $entity_name . '_id' ] = $post_id;
	        $section['content'] = '';

	        if ( ! empty( $section['frontend_name'] ) ) {
		        $section['label'] = $section['frontend_name'];
	        }

	        if ( ! empty( $section['frontend_name_callback'] ) ) {
	            $section['label'] = call_user_func( $section['frontend_name_callback'] );
            }
            ...
            } else if ( function_exists( 'es_the_entity_section_content' ) ) {
	            $section['content'] = call_user_func( 'es_the_entity_section_content', $section, $post_id );
            }

            $locate_template = es_locate_template( 'front/' . $entity_name . '/section.php' );

            if ( file_exists( $locate_template ) ) {
                es_load_template( 'front/' . $entity_name . '/section.php', $section );
            } else {
	            es_load_template( 'front/entity/partials/section.php', $section );
            }
...
if ( ! function_exists( 'es_listings_categories' ) ) {

	/**
	 * @return void
	 */
    function es_listings_categories( $args ) {
        /** @var WP_Query $query */
	    $query = ! empty( $args['query'] ) ? $args['query'] : null;

	    if ( $query && $query->have_posts() && ! empty( $args['show_categories'] ) ) {
	        ...
	        if ( $categories && ! is_wp_error( $categories ) ) {
	            ...
	            unset( $query_args['tax_query']['es_category'] );

	            $count_all_query = new WP_Query( $query_args );
	            $args['count_all_posts'] = $count_all_query->found_posts;

	            foreach ( $categories as $key => $term ) {
                    $tax_query = ! empty( $query_args['tax_query'] ) ? $query_args['tax_query'] : array();
                    $tax_query = $tax_query ? $tax_query : array();
                    $tax_query['es_category'] = array(
	                    'taxonomy' => 'es_category',
	                    'terms' => $term->term_id,
	                    'field' => 'id'
                    );
                    ...
                    $counter_query = new WP_Query( $query_args );

                    if ( $counter_query->found_posts ) {
	                    $categories[ $key ]->count = $counter_query->found_posts;
                    } else {
	            ...
	            if ( ! empty( $categories ) ) {
		            ...
		            es_load_template( 'front/property/partials/properties-categories.php', $args );
                }
            }
	    }
    }
}
add_action( 'es_before_listings', 'es_listings_categories' );
...
if ( ! function_exists( 'es_get_properties_query_args' ) ) {
    ...
    function es_get_properties_query_args( $args = array() ) {
        ...
        $search_context = ! empty( $args['fields']['search_context'] ) ? $args['fields']['search_context'] : null;
        if ( isset( $args['fields']['search_context'] ) ) {
	        unset( $args['fields']['search_context'] );
        }
        ...
        if ( ! empty( $args['fields'] ) ) {
            foreach ( $args['fields'] as $field => $value ) {
                ...
                if ( $search_context == 'pm' && $field == 'address' ) {
                    $field = 'keywords';
                    $value = array( $value );
                }
...
add_action( 'es_entities_no_found_posts', 'es_properties_no_found_posts' );
...
if ( ! function_exists( 'es_the_property_breadcrumbs' ) ) {
	...
	/**
     * Display breadcrumbs on single property page.
     *
	 * @param int $post_id
	 */
 ...
 * @param $post_id
 */
function es_property_content_extended() {
    es_load_template( 'front/property/partials/content-archive-extra.php' );
}
